...
def parse_args():
    ...
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=Path("checkpoints/split_sae/main_ladv1_ldom1/best_recon.pt"),
    )
    parser.add_argument(
        "--scaler",
        type=Path,
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/split_sae_latent_probes"))
...
def extract_latents(model, dataset, batch_size, num_workers, device):
    ...
    with torch.no_grad():
        for batch in tqdm(loader, leave=False):
            ...
            batch_recon_mse = ((reconstruction - embeddings) ** 2).mean()
            batch_good_recon_mse = ((good_reconstruction - embeddings) ** 2).mean()
            batch_badcon_loss = (bad_reconstruction ** 2).mean()
            total_recon_mse += batch_recon_mse.item() * batch_size
            total_good_recon_mse += batch_good_recon_mse.item() * batch_size
            total_badcon_loss += batch_badcon_loss.item() * batch_size
...
def main():
    ...
    if not args.checkpoint.exists():
        raise FileNotFoundError(f"Missing split SAE checkpoint: {args.checkpoint}")
    ...
    print("")
    print(f"Saved split-SAE latent probe results to {results_path}")
