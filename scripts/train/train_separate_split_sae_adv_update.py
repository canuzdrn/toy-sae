"""Train separate-encoder split SAE with extra adversary-head updates per batch."""
...
def run_epoch(model, loader, main_optimizer, head_optimizer, device, args, training):
    ...
    with context:
        for batch in loader:
            ...
            good_stats = latent_stats(outputs["z_good"])
            bad_stats = latent_stats(outputs["z_bad"])
    ...
    torch.save(
        {
            ...
            "args": args_to_dict(
                args,
                extra={
                    "model_name": "separate_encoder_sae",
                    "training_procedure": "adversary_head_updates",
                },
            ),
        },
        path,
