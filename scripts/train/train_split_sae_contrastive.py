"""Train shared-trunk split SAE with contrastive color-invariance on z_good.

This keeps the existing adversary-head-update training objective intact and adds
one paired contrastive training constraint:

    z_good(same image colored red) is close to z_good(same image colored green)
    z_good(different images in the batch) are pushed apart
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Optional MSE penalty between red/green z_good pairs. Logged regardless.",
    )
    parser.add_argument("--lambda-contrast-good", type=float, default=0.05)
...
def pair_metrics(model, red_embeddings, green_embeddings, temperature):
    ...
    normalized_red = F.normalize(z_good_red, dim=1)
    normalized_green = F.normalize(z_good_green, dim=1)
    logits = normalized_red @ normalized_green.T / temperature
    labels = torch.arange(logits.shape[0], device=logits.device)
    red_to_green = F.cross_entropy(logits, labels)
    green_to_red = F.cross_entropy(logits.T, labels)
    good_contrastive_loss = 0.5 * (red_to_green + green_to_red)

    return {
        "good_pair_mse": good_pair_mse,
        "bad_pair_mse": bad_pair_mse,
        "good_pair_cosine": good_pair_cosine,
        "bad_pair_cosine": bad_pair_cosine,
        "good_contrastive_loss": good_contrastive_loss,
    }
...
def run_epoch(model, loader, main_optimizer, head_optimizer, device, args, training):
    ...
    with context:
        for batch in loader:
            ...
            total_loss = (
                base_total_loss
                + args.lambda_pair_good * paired["good_pair_mse"]
                + args.lambda_contrast_good * paired["good_contrastive_loss"]
            )
            ...
            for key in [
                ...
                "good_pair_cosine",
                "bad_pair_cosine",
                "good_contrastive_loss",
            ]:
                totals[key] += paired[key].item() * batch_size
    ...
    torch.save(
        {
            ...
            "args": args_to_dict(args, extra={"training_procedure": "contrastive_good_invariance"}),
        },
        path,
    )
...
def main():
    ...
    print("Training procedure: adversary-head updates + contrastive z_good invariance")
    ...
    print(f"lambda_contrast_good={args.lambda_contrast_good}")
    print(f"contrast_temperature={args.contrast_temperature}")
    ...
    config = args_to_dict(args, extra={"training_procedure": "contrastive_good_invariance"})
    save_json(config_path, config)
    save_scaler(scaler_path, train_dataset)

    best_val_recon = float("inf")
    best_val_total = float("inf")
    best_val_contrast = float("inf")
    history = []
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        message = (
            ...
            f"val_contrast={val_metrics['good_contrastive_loss']:.6f} "
        ...
        if val_metrics["good_contrastive_loss"] < best_val_contrast:
            best_val_contrast = val_metrics["good_contrastive_loss"]
            save_checkpoint(
                args.checkpoint_dir / "best_contrast.pt",
                model,
                main_optimizer,
                head_optimizer,
                args,
            ...
            print(
                "  saved new best contrastive-invariance checkpoint to "
                f"{args.checkpoint_dir / 'best_contrast.pt'}"
            )
